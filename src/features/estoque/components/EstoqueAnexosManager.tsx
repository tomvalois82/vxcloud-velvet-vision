...
// Helper to get file source (priority: base64, then url)
...
interface EstoqueAnexosManagerProps {
...
export function EstoqueAnexosManager({ estoqueId }: EstoqueAnexosManagerProps) {
  ...
  const compressFile = async (file: File): Promise<File> => {
    ...
    if (file.size > MAX_SIZE_BYTES * 3) {
      toast({
        title: "Aviso",
        description: `O arquivo "${file.name}" é grande (${(file.size / 1024 / 1024).toFixed(1)}MB). O upload pode demorar.`,
      });
    }
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files.length || !estoqueId) return;

    // Check max files limit
    if (anexos.length + files.length > MAX_FILES) {
      toast({
        title: "Limite excedido",
        description: `Máximo de ${MAX_FILES} arquivos por veículo.`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      for (const file of validFiles) {
        ...
        const { error: dbError } = await supabase.from("estoque_anexos").insert({
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      // If has url_arquivo, try to delete from storage
      if (anexoToDelete.url_arquivo) {
        const urlParts = anexoToDelete.url_arquivo.split("/");
        const pathIndex = urlParts.findIndex((p) => p === "estoque");
        if (pathIndex !== -1) {
